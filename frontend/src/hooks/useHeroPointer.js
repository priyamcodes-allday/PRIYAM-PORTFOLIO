  ...
  useEffect(
    () =>
      pointerMotion((finePointer) => {
        ...
        const endTouch = (event) => {
          ...
          if (gesture.horizontal) reset();
            ...
            releaseTimer = window.setTimeout(
              reset,
              Math.max(0, 160 - (performance.now() - gesture.time)),
            );
        };
        const leave = (event) => {
        ...
        portrait.addEventListener("pointerenter", movePortrait, {
          passive: true,
        });
        ...
        return () => {
          ...
          ["--photo-rotation", "--photo-x", "--photo-y"].forEach((name) =>
            portrait.style.removeProperty(name),
          );
        };
      }),
    [],
